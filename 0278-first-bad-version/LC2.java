...
-----------------------------------------------------
 ...
 |       |       |
left    mid    right
Let us look at the first scenario above where isBadVersion(mid)⇒false. We know that all versions preceding and including midmidmid are all good. So we set left=mid+1 to indicate that the new search space is the interval [mid+1,right] (inclusive).
...
------------------------------------------------------
 ...
 |       |       |
left    mid    right
- The only scenario left is where isBadVersion(mid)⇒true. This tells us that mid may or may not be the first bad version, but we can tell for sure that all versions after midmidmid can be discarded. Therefore we set right=mid as the new search space of interval [left,mid] (inclusive).
