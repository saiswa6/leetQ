...
Time complexity: O(n⋅2n)O(n \cdot 2 ^ n)O(n⋅2 
n
 )

Sorting the nums array requires nlog⁡nn \log nnlogn time. The outer for loop runs 2n2 ^ n2 
n
  times. The inner loop runs nnn times. We know that average case time complexity for set operations is O(1)O(1)O(1). Although, to generate a hash value for each subset O(n)O(n)O(n) time will be required. However, we are generating the hashcode while iterating the inner for loop. So the overall time complexity is O(nlog⁡n+2n⋅(nO(n \log n + 2^n \cdot (nO(nlogn+2 
n
 ⋅(n (for inner loop) + nnn (for stringbuilder to string conversion in Java) )))))) = O(2n⋅n)O(2 ^ n \cdot n)O(2 
n
 ⋅n).

Space complexity: O(n⋅2n)O(n \cdot 2^n)O(n⋅2 
n
 )

We need to store at most 2n2 ^ n2 
n
...
The space required for the output list is not considered while analyzing space complexity. Thus the overall space complexity in Big O Notation is O(n⋅2n)O(n \cdot 2^n)O(n⋅2 
n
 ).
