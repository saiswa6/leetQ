class Solution {
    ...
    /*public boolean isValidPalindrome(String s, int k) {
        ...
        while(start < end) {
            if(s.charAt(start) != s.charAt(end) && count > 0) {
                return checkPalindrome(s, start, end, count);
            }
            start++;
            end--;
        }
    ...
    public boolean checkPalindrome(String s, int start, int end, int count) {
        while(start < end) {
            if(s.charAt(start) == s.charAt(end)) {
                start++;
                end--;
            } else if(s.charAt(start) != s.charAt(end) & count > 0) {
                return (checkPalindrome(s, start + 1, end, count - 1) || checkPalindrome(s, start , end - 1, count - 1)) ;
            } else {
                return false;
            }
        }
